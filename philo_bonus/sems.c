...
bool	open_simulation_sems(t_table *table)
{
	...
	table->full = sem_open("FULL_COUNT", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR,
			0);
	...
	table->death = sem_open("DEATH_FLAG", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR,
			0);
...
{
	sem_unlink("FORK_SEM");
	table->forks = sem_open("FORK_SEM", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR,
			table->phil_count);
