...
/*   Updated: 2025/07/31 18:57:26 by amagno-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int init_mutexes(t_data *data)
{
    ...
    data->table.forks = malloc(sizeof(pthread_mutex_t) * data->table.phil_count);
    if (!data->table.forks)
        return (0);
...
int start_simulation(t_data *data)
{
    ...
    while (++i < data->table.phil_count)
    {
        if (pthread_create(&data->table.phil[i].thread, NULL, 
                          phil_life, &data->table.phil[i]) != 0)
            return (0);
    }
